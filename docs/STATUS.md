...
| Phase 3 — Implementation | S4 — S9 | S4 done; S5 implementation done (RC tag pending); S6 in progress (8/18, after RoutingPolicy decomposition); S7 — S9 pending | per-sprint table below |
...
| S6 — L3 family | 18 resources after RoutingPolicy decomposition: `Loopback`, `Vrf`, `Bfd`, `Subinterface`, `StaticRoute`, `RouterBgp`, `PrefixList`, `RouteMap`, `CommunityList`, `ExtCommunityList`, `AsPathAccessList`, `Rcf`, `Rpki`, `RouterOspf`, `GreTunnel`, `Vrrp`, `PolicyBasedRouting`, `ResilientEcmp` (see Tier 2 ordering) | in progress (8/18) | shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (`eea0e74`), `Subinterface` (`8b2dd6e`), `StaticRoute` (`908af76`), `RouterBgp` v0 (`1574102`), `PrefixList` (`11a4cd6`), `RouteMap` (this commit); 24/24 cEOS integration tests pass |
...
| **Tier 2 — Open S6 (L3 critical path)** | ~~`Loopback`~~ → ~~`Vrf`~~ → ~~`Bfd`~~ → ~~`Subinterface`~~ → ~~`StaticRoute`~~ → ~~`RouterBgp`~~ (v0) → ~~`PrefixList`~~ → ~~`RouteMap`~~ → `CommunityList` → `ExtCommunityList` → `AsPathAccessList` → `Rcf` → `Rpki` → `RouterOspf` → `GreTunnel` → `Vrrp` → `PolicyBasedRouting` → `ResilientEcmp`. | 10 items remaining | Tier 2 expanded after RoutingPolicy decomposition. RouteMap shipped with full match (ip address prefix-list / community / extcommunity / as-path / interface / tag / metric / local-preference / origin / source-protocol) and set (community / additive / none / extcommunity rt / local-preference / metric / as-path prepend / ip next-hop / origin / tag) vocabulary. |
...
| Commit | Subject | Date |
|---|---|---|
| pending | `feat(l3): eos:l3:RouteMap with full match/set vocabulary` | 2026-04-30 |
