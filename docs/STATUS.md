...
| Phase 3 — Implementation | S4 — S9 | S4 done; S5 implementation done (RC tag pending); S6 in progress (3/14); S7 — S9 pending | per-sprint table below |
...
| S6 — L3 family | `Loopback`, `Vrf`, `Bfd`, `Interface` (routed), `StaticRoute`, `RouterBgp` (peer-groups + per-AF + per-VRF + EVPN AF + RD/RT), `RoutingPolicy`, `Rcf`, `Rpki`, `RouterOspf`, `GreTunnel`, `Vrrp`, `PolicyBasedRouting`, `ResilientEcmp` (14 resources, see Tier 2 ordering) | in progress (3/14) | shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (this commit); 19/19 cEOS integration tests pass |
