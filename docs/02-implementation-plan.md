...
| **S6** | L3 family: `Loopback`, `Vrf`, `Bfd`, `Subinterface`, `StaticRoute`, `RouterBgp` (peer-groups, per-AF, per-VRF, RD/RT, RCF), `RouterOspf`, `PrefixList`, `RouteMap`, `CommunityList`, `ExtCommunityList`, `AsPathAccessList`, `Rcf`, `Rpki`, `GreTunnel`, `Vrrp`, `ResilientEcmp`, `PolicyBasedRouting`. | Leaf-spine demo (2 leaves + 1 spine, EVPN A-A) deploys end-to-end against cEOS. **Done (18/18) — shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (`eea0e74`), `Subinterface` (`8b2dd6e`), `StaticRoute` (`908af76`), `RouterBgp` v0 (`1574102`), `PrefixList` (`11a4cd6`), `RouteMap` (`d50fc2a` + additive-flag fix `72c7db5` + `Match.Origin` removal `8d7ea48`), `CommunityList` (`1566544`), `ExtCommunityList` (`51416dc`), `AsPathAccessList` (`888a110`), `Rcf` v1 (`8e02a46`, supersedes v0 `fbdfbb5`), `Rpki` (`1e93356` + `transport ssh` → `tls` correction), `RouterOspf` v0 (`a059ec1`), `GreTunnel` v0 (`d2ee58a` + `mpls-*` mode removal `8d7ea48`), `Vrrp` v0 (`9d28327`), `ResilientEcmp` v0 (`e6606fe`, platform-only — TOI 13938), `PolicyBasedRouting` v0 (`4bc171a`, ACL name as string until S7 Tier 3.1); 34/34 cEOS integration tests pass. Cross-resource keyword audit (`make probe-audit`) at 95/98 surface lines accepted across 14 l3 surfaces. **Tier 2 closed.** Next: Tier 3.0 (S7 device-foundation).** |
| **S7** | Security & management & multicast & QoS — 46 resources organised by **dependency depth** into Tier 3.0 → 3.7 (see `docs/STATUS.md` §Priority ordering and `docs/01-architecture.md` §Resource-dependency graph). Phase 3.0 (device-foundation: `Hostname`, `NtpServer`, `DnsServer`, `Logging`, `EApi`, `ManagementInterface` — zero deps) → 3.1 (unblockers: `IpAccessList` + `Ipv6AccessList` + `MacAccessList` + `SslProfile` + `Role` — each closes 2+ downstream gaps) → 3.2 (AAA: `UserAccount`, `RoleBasedAccessList`, `AaaServer`, `AaaAuthentication`) → 3.3 (sec-core: `ControlPlanePolicing`, `Urpf`, `ServiceAcl`, `MacSecProfile`, `MacSecBinding`) → 3.4 (campus, parallel: `Dot1x`, `Mab`, `Pvlan`, `StormControl`, `DhcpRelay`, `DhcpSnooping`, `DynamicArpInspection`, `IpSourceGuard`, `ArpRateLimit`) → 3.5 (multicast: `Igmp`, `IgmpSnooping`, `Pim`, `AnycastRp`, `Msdp`, `MulticastRoutingTable`) → 3.6 (QoS: `ClassMap` → `PolicyMap` → `ServicePolicy`, plus `QosMap`, `PriorityFlowControl`, `BufferProfile`) → 3.7 (mgmt-extras, parallel: `Snmp`, `Sflow`, `Telemetry`, `EventMonitor`, `PortMirror`). | Management, security, multicast, QoS planes covered; AAA / RBAC documented. **Closes audit gaps** from S6: `RouteMap.match ip address access-list` (needs `IpAccessList` from 3.1); RPKI TLS run-time path (needs `SslProfile` from 3.1); PBR ACL coupling hardens from string fallback to typed reference. |
