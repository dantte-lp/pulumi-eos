...
| S5 — L2 family | `Vlan`, `VlanInterface`, `Interface`, `PortChannel`, `EvpnEthernetSegment`, `Mlag`, `VxlanInterface`, `Stp`, … | in progress | `Vlan`, `VlanInterface`, `Interface` shipped (CRUD + Diff over eAPI config-session, integration-tested on cEOS). |
...
| `_pending_` | `feat(l2): eos:l2:Interface (physical Ethernet) resource` | 2026-04-30 |
