...
| ~~Audit gap — `eos:l3:Rpki` `transport ssh`~~ corrected to `tls` (this commit) | Bug — `transport ssh` was never a valid EOS keyword. Per TOI 14470 §Limitations: "The only transports supported for connecting to the RPKI cache are unprotected TCP and TLS." Validator now accepts `{tcp, tls}`; renaming reflected in CHANGELOG / tests / cspell. | — |
...
| Stretch — eAPI error → typed sentinel mapping | `EOS-4.36.0.1F-SysMsgGuide.csv` (extracted from user-supplied archive — 2 747 syslog patterns: Facility / Mnemonic / Format / Severity / Explanation / Recommended Action) lives at `/opt/software/arista-extracted/EOS-4.36.0.1F/`. Future task: build a Go `sysmsg` lookup table inside `internal/client/eapi/` so eAPI error envelopes can be matched to typed sentinels rather than regex string-matching. | post-S6 stretch |
