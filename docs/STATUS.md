...
| Phase 3 — Implementation | S4 — S9 | S4 done; S5 implementation done (RC tag pending); S6 in progress (5/14); S7 — S9 pending | per-sprint table below |
...
| **Tier 2 — Open S6 (L3 critical path)** | ~~`Loopback`~~ → ~~`Vrf`~~ → ~~`Bfd`~~ → ~~`Subinterface`~~ → ~~`StaticRoute`~~ → `RouterBgp` (peer-groups + per-AF + per-VRF + EVPN AF + RD/RT) → `RoutingPolicy` → `Rcf` → `Rpki` → `RouterOspf` → `GreTunnel` → `Vrrp` → `PolicyBasedRouting` → `ResilientEcmp`. | 9 items remaining | `RouterBgp` is the single largest resource in the project; everything from underlay reachability to overlay EVPN flows through it. The first five resources cover the underlay plumbing — RouterBgp is unblocked. |
