...
| Phase 3 — Implementation | S4 — S9 | S4 done; S5 implementation done (RC tag pending); S6 in progress (9/18, after RoutingPolicy decomposition); S7 — S9 pending | per-sprint table below |
...
| S6 — L3 family | 18 resources after RoutingPolicy decomposition: `Loopback`, `Vrf`, `Bfd`, `Subinterface`, `StaticRoute`, `RouterBgp`, `PrefixList`, `RouteMap`, `CommunityList`, `ExtCommunityList`, `AsPathAccessList`, `Rcf`, `Rpki`, `RouterOspf`, `GreTunnel`, `Vrrp`, `PolicyBasedRouting`, `ResilientEcmp` (see Tier 2 ordering) | in progress (9/18) | shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (`eea0e74`), `Subinterface` (`8b2dd6e`), `StaticRoute` (`908af76`), `RouterBgp` v0 (`1574102`), `PrefixList` (`11a4cd6`), `RouteMap` (`d50fc2a`), `CommunityList` (this commit); 25/25 cEOS integration tests pass |
...
| **Tier 2 — Open S6 (L3 critical path)** | ~~`Loopback`~~ → ~~`Vrf`~~ → ~~`Bfd`~~ → ~~`Subinterface`~~ → ~~`StaticRoute`~~ → ~~`RouterBgp`~~ (v0) → ~~`PrefixList`~~ → ~~`RouteMap`~~ → ~~`CommunityList`~~ → `ExtCommunityList` → `AsPathAccessList` → `Rcf` → `Rpki` → `RouterOspf` → `GreTunnel` → `Vrrp` → `PolicyBasedRouting` → `ResilientEcmp`. | 9 items remaining | RoutingPolicy 5-way decomposition — 3 of 5 atomic resources shipped (PrefixList, RouteMap, CommunityList); ExtCommunityList + AsPathAccessList follow. cEOS 4.36 quirk discovered + documented: `regexp` keyword (not `expanded` per User Manual) for the regex form. |
...
| Commit | Subject | Date |
|---|---|---|
| pending | `feat(l3): eos:l3:CommunityList (standard + regexp; cEOS 4.36 keyword quirk)` | 2026-04-30 |
