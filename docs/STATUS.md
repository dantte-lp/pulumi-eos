...
| S6 — L3 family | 18 resources after RoutingPolicy decomposition: `Loopback`, `Vrf`, `Bfd`, `Subinterface`, `StaticRoute`, `RouterBgp`, `PrefixList`, `RouteMap`, `CommunityList`, `ExtCommunityList`, `AsPathAccessList`, `Rcf`, `Rpki`, `RouterOspf`, `GreTunnel`, `Vrrp`, `PolicyBasedRouting`, `ResilientEcmp` (see Tier 2 ordering) | in progress (16/18); RoutingPolicy 5/5 done | shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (`eea0e74`), `Subinterface` (`8b2dd6e`), `StaticRoute` (`908af76`), `RouterBgp` v0 (`1574102`), `PrefixList` (`11a4cd6`), `RouteMap` (`d50fc2a` + additive-flag fix `72c7db5`), `CommunityList` (`1566544`), `ExtCommunityList` (`51416dc`), `AsPathAccessList` (`888a110`), `Rcf` v1 (`8e02a46`, supersedes v0 `fbdfbb5`), `Rpki` (`1e93356`), `RouterOspf` v0 (`a059ec1`), `GreTunnel` v0 (`d2ee58a`), `Vrrp` v0 (this commit); 33/33 cEOS integration tests pass |
...
| **Tier 2 — Open S6 (L3 critical path)** | ~~`Loopback`~~ → ~~`Vrf`~~ → ~~`Bfd`~~ → ~~`Subinterface`~~ → ~~`StaticRoute`~~ → ~~`RouterBgp`~~ (v0) → ~~`PrefixList`~~ → ~~`RouteMap`~~ → ~~`CommunityList`~~ → ~~`ExtCommunityList`~~ → ~~`AsPathAccessList`~~ → ~~`Rcf`~~ v1 → ~~`Rpki`~~ → ~~`RouterOspf`~~ (v0) → ~~`GreTunnel`~~ (v0) → ~~`Vrrp`~~ (v0) → `PolicyBasedRouting` → `ResilientEcmp`. | 2 items remaining | Vrrp v0 covers inline-form `vrrp <vrid> <subcommand>` per User Manual §17. v0 surface: identity (interface + vrid), virtual addresses (IPv4 primary + secondary, IPv6), priority-level, preempt + delay-minimum, advertisement interval, session description, bfd ip, disabled. EOS keyword corrections caught at integration time: `priority` → `priority-level` (§14.5.4.21), `timers advertise` → `advertisement interval` (§14.5.4.10), `description` → `session description` (§14.5.4.22), `bfd peer` → `bfd ip` (§16.7.4.22), `shutdown` → `disabled` (§14.5.2.1.3). Tracks (input-shape only) defer rendering to a future `eos:l3:Tracker` resource that owns the global `track` namespace. |
...
| Commit | Subject | Date |
|---|---|---|
| `<this>` | `feat(l3): eos:l3:Vrrp v0 — VRRP virtual router (inline form)` | 2026-05-01 |
