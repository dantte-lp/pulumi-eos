...
| Phase 3 — Implementation | S4 — S9 | S4 done; S5 implementation done (RC tag pending); **S6 done (18/18 — Tier 2 closed)**; S7 — S9 pending | per-sprint table below |
...
| S6 — L3 family | 18 resources after RoutingPolicy decomposition: `Loopback`, `Vrf`, `Bfd`, `Subinterface`, `StaticRoute`, `RouterBgp`, `PrefixList`, `RouteMap`, `CommunityList`, `ExtCommunityList`, `AsPathAccessList`, `Rcf`, `Rpki`, `RouterOspf`, `GreTunnel`, `Vrrp`, `ResilientEcmp`, `PolicyBasedRouting` (see Tier 2 ordering) | **done (18/18)**; RoutingPolicy 5/5 done | shipped: `Loopback` (`5dcc4df`), `Vrf` (`c335d22`), `Bfd` (`eea0e74`), `Subinterface` (`8b2dd6e`), `StaticRoute` (`908af76`), `RouterBgp` v0 (`1574102`), `PrefixList` (`11a4cd6`), `RouteMap` (`d50fc2a` + additive-flag fix `72c7db5`), `CommunityList` (`1566544`), `ExtCommunityList` (`51416dc`), `AsPathAccessList` (`888a110`), `Rcf` v1 (`8e02a46`, supersedes v0 `fbdfbb5`), `Rpki` (`1e93356`), `RouterOspf` v0 (`a059ec1`), `GreTunnel` v0 (`d2ee58a`), `Vrrp` v0 (`9d28327`), `ResilientEcmp` v0 (`e6606fe`; platform-only — cEOSLab returns "Unavailable command", production EOS DCS-7050/7160/7280/73xx/75xx/78xx accepts directly per TOI 13938), `PolicyBasedRouting` v0 (`4bc171a`; ACL name as string until S7 Tier 3.1 ships `eos:security:IpAccessList`); 34/34 cEOS integration tests pass |
...
| Cross-resource keyword audit | `make probe-audit` — Python harness under `tools/probe_audit/` (uv + ruff + ty) | last run: 95/98 lines accepted across 14 l3 surfaces. Not-OK breakdown: 2 PBR FAIL (`set nexthop … recursive` not in EOS 4.36; `set ipv6 nexthop` wrong syntax — replacement is `set nexthop <ipv6-addr>`; both documented in resource Annotate); 1 PLAT (cEOSLab `tunnel dont-fragment` — production EOS hardware accepts). Caught + closed: `RouteMap.Match.Origin`, `GreTunnel.Mode={mpls-gre,mpls-over-gre}` (commit `8d7ea48`). |
...
| `eos:l3` | 20 | 18 | 2 | 90% | S6 closed; `RouterIsis` + `Nat` post-S9 stretch |
...
| ~~Audit gap — `eos:l3:RouteMap` `match origin` field~~ removed | Verified via arista-mcp: EOS does NOT have `match origin` (only `set origin`). Use `match route-type` (TOI 13916). Closed by `8d7ea48`. | — |
| ~~Audit gap — `eos:l3:GreTunnel` `mpls-gre` / `mpls-over-gre` modes~~ removed | Verified via arista-mcp + TOI 13806: MPLS-over-GRE is configured via `nexthop-group type mpls-over-gre`, NOT as a `tunnel mode` keyword. Validator now caps mode to `{gre, ipsec}`. Future `eos:l3:NexthopGroupMplsOverGre` resource will model the actual feature. Closed by `8d7ea48`. | — |
| ~~Audit gap — `eos:l3:Rpki` `transport ssh`~~ corrected to `tls` | Bug — `transport ssh` was never a valid EOS keyword. Per TOI 14470 §Limitations: "The only transports supported for connecting to the RPKI cache are unprotected TCP and TLS." Validator now accepts `{tcp, tls}`; renaming reflected in CHANGELOG / tests / cspell. | — |
| Platform-conditional surfaces (cEOSLab quirks, work on hardware) | Documented in resource Annotate strings; integration body skips them. Affected: `eos:l3:GreTunnel.DontFragment`, `eos:l3:GreTunnel.Mode=ipsec`, `eos:l3:Subinterface.Mtu`, `eos:l3:Rpki.Transport=tls`. Production EOS (DCS-7280R*, 7500R*, 7050X* etc.) accepts all four; cEOSLab returns "Unavailable command (not supported on this hardware platform)" at commit. | — |
| Stretch — eAPI error → typed sentinel mapping | Source data: `EOS-4.36.0.1F-SysMsgGuide.csv` (2 747 rows: Facility / Mnemonic / Format / Severity / Explanation / Recommended Action). Future surface: Go `sysmsg` lookup table inside `internal/client/eapi/`; eAPI error envelopes match a typed sentinel instead of regex string-matching. | post-S6 stretch |
| vEOS-lab bring-up — partial | qcow2 + Aboot ISO mounted in `compose.veos.yml`; Aboot drives the boot, EOS reaches "Welcome … 4.36.0.1F" → init stage 2 reliably. Still hits the ZTP loop because the `zerotouch-config DISABLE` token on the secondary FAT32 USB disk is not auto-copied into `/mnt/flash` by EOS. Next-step (tracked separately): vrnetlab-style `launch.py` that does the disk-layout dance correctly, OR pre-bake the disable token into the qcow2 layer. cEOS-lab remains the default integration target — same Arfa fast-path so surface coverage is identical. | post-S6 stretch |

## Repository activity

| Commit | Subject | Date |
|---|---|---|
| `4bc171a` | `feat(l3): eos:l3:PolicyBasedRouting v0 — closes Tier 2 (S6 18/18)` | 2026-05-01 |
| `ca5f638` | `docs(plan): re-sequence S7 by dependency depth into Tier 3.0 → 3.7` | 2026-05-01 |
| `a159a03` | `docs(emulators): research EOS validation targets; add vEOS-lab compose (WIP)` | 2026-05-01 |
| `e6606fe` | `feat(l3): eos:l3:ResilientEcmp v0 — slot-table binding (platform-only)` | 2026-05-01 |
| `7e6d8c3` | `fix(l3): Rpki transport ssh→tls; reclassify cEOSLab quirks; reorder Tier-2` | 2026-05-01 |
| `7749489` | `docs(sync): align all docs to S6 16/18 + probe-audit harness` | 2026-05-01 |
...
| `3c13006` | `docs(rules): single CLI-validation channel via internal/client/eapi` | 2026-05-01 |
| `1e93356` | `feat(l3): eos:l3:Rpki — RPKI cache config under router bgp` | 2026-05-01 |
